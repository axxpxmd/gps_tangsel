...
<div>
    ...
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-6">
        ...
        <div class="flex flex-col sm:flex-row items-stretch sm:items-center gap-2 w-full sm:w-auto">
            <form action="{{ route('console.article-old.copy') }}" method="POST" onsubmit="return confirm('Salin semua artikel dari WordPress ke tabel artikel? Artikel yang sudah ada akan dilewati.')" class="flex-1 sm:flex-none">
                @csrf
                <button type="submit" class="w-full inline-flex items-center justify-center gap-2 px-4 py-2.5 bg-emerald-500 text-white text-sm font-semibold rounded-xl hover:bg-emerald-600 transition-colors duration-200">
            ...
            <form action="{{ route('console.article-old.fetch') }}" method="POST" onsubmit="return confirm('Tarik data terbaru dari WordPress?')" class="flex-1 sm:flex-none">
                @csrf
                <button type="submit" class="w-full inline-flex items-center justify-center gap-2 px-4 py-2.5 bg-primary text-white text-sm font-semibold rounded-xl hover:bg-primary-dark transition-colors duration-200">
                    <svg class="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
        ...
        <div class="bg-white rounded-2xl border border-gray-200 p-5">
            <div class="flex items-center gap-3">
                ...
                <div>
                    ...
                    <p class="text-[11px] text-gray-400 font-medium">Total WP</p>
                </div>
            </div>
        </div>
        <div class="bg-white rounded-2xl border border-gray-200 p-5">
            <div class="flex items-center gap-3">
                ...
                <div>
                    <p class="text-2xl font-extrabold text-gray-900">{{ $copiedCount }}</p>
                    <p class="text-[11px] text-gray-400 font-medium">Sudah Disalin</p>
                </div>
            </div>
        </div>
        <div class="bg-white rounded-2xl border border-gray-200 p-5">
                            ...
                            <tr class="hover:bg-gray-50/50 transition-colors duration-150">
                                ...
                                <td class="px-5 py-4">
                                    <div class="min-w-0">
                                        ...
                                        @if ($article->excerpt)
                                            <p class="text-xs text-gray-400 mt-0.5 line-clamp-1">{{ Str::limit(strip_tags($article->excerpt), 60) }}</p>
                                        @endif
                                    </div>
                                </td>
                                ...
                                <td class="px-5 py-4 text-right">
                                    <div class="flex items-center justify-end gap-1">
                                        <a href="{{ route('console.article-old.show', $article) }}" class="p-2 rounded-lg text-gray-400 hover:text-primary hover:bg-primary/10 transition-colors duration-200" title="Lihat">
                                            <svg class="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
            ...
            @foreach ($articles as $article)
                <div class="bg-white rounded-2xl border border-gray-200 p-4">
                    <div class="flex items-start justify-between gap-2 mb-2">
                        <a href="{{ route('console.article-old.show', $article) }}" class="text-sm font-bold text-gray-900 hover:text-primary transition-colors duration-200 line-clamp-2">{{ $article->title }}</a>
                        @if ($article->status === 'publish')
                    ...
                    @if ($article->excerpt)
                        <p class="text-xs text-gray-400 line-clamp-2 mb-2">{{ Str::limit(strip_tags($article->excerpt), 80) }}</p>
                    @endif
                    <div class="flex items-center justify-between">
                        <span class="text-[11px] text-gray-500">{{ $article->wp_created_at?->translatedFormat('d M Y') ?? '—' }}</span>
                        <a href="{{ route('console.article-old.show', $article) }}" class="p-1.5 rounded-lg text-gray-400 hover:text-primary hover:bg-primary/10 transition-colors duration-200" title="Lihat">
                            <svg class="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
                                <path stroke-linecap="round" stroke-linejoin="round" d="M2.036 12.322a1.012 1.012 0 010-.639C3.423 7.51 7.36 4.5 12 4.5c4.638 0 8.573 3.007 9.963 7.178.07.207.07.431 0 .639C20.577 16.49 16.64 19.5 12 19.5c-4.638 0-8.573-3.007-9.963-7.178z"/>
                                <path stroke-linecap="round" stroke-linejoin="round" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z"/>
                            </svg>
