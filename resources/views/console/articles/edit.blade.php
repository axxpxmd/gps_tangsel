...
<div>
    ...
    <form action="{{ route('console.articles.update', $article) }}" method="POST" enctype="multipart/form-data" x-data="{ previewUrl: '{{ $article->image }}', currentImage: '{{ $article->image }}' }">
        @csrf
        @method('PUT')
            ...
            <div class="xl:col-span-2 space-y-5">
                ...
                <div class="bg-white rounded-2xl border border-gray-200 p-5 sm:p-6">
                    ...
                    <textarea id="excerpt" name="excerpt" rows="4" required
                        ...
                        placeholder="Tulis ringkasan singkat yang menggambarkan isi artikel...">{{ old('excerpt', $article->excerpt) }}</textarea>
                    @error('excerpt')
                        <p class="text-xs text-red-500 mt-1.5 flex items-center gap-1">
                            <svg class="w-3.5 h-3.5" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2"><path stroke-linecap="round" stroke-linejoin="round" d="M12 9v3.75m9-.75a9 9 0 11-18 0 9 9 0 0118 0zm-9 3.75h.008v.008H12v-.008z"/></svg>
                            {{ $message }}
            ...
            <div class="space-y-5">
                ...
                <div class="bg-white rounded-2xl border border-gray-200 p-5 sm:p-6">
                    ...
                    <div class="mb-3">
                        ...
                        <template x-if="previewUrl">
                            <div class="relative aspect-[16/9] rounded-xl overflow-hidden bg-gray-100">
                                ...
                                <button type="button" @click="previewUrl = null; currentImage = null; document.getElementById('image').value = ''"
                    ...
                    <input type="file" id="image" name="image" accept="image/jpeg,image/png,image/webp"
                        @change="previewUrl = $el.files.length ? URL.createObjectURL($el.files[0]) : currentImage || null"
        ...
        <div class="bg-white rounded-2xl border border-gray-200 p-4 sm:p-5 flex flex-col sm:flex-row items-center justify-between gap-3">
            <div class="flex items-center gap-2 text-xs text-gray-400">
                ...
                <span>Perubahan akan disimpan dan langsung diterapkan.</span>
            </div>
            <div class="flex items-center gap-3">
                <a href="{{ route('console.articles.index') }}" class="inline-flex items-center gap-1.5 px-5 py-2.5 text-sm font-medium text-gray-500 hover:text-gray-900 hover:bg-gray-100 rounded-xl transition-colors duration-200">Batal</a>
                <button type="submit" class="inline-flex items-center gap-2 px-6 py-2.5 bg-gradient-to-r from-primary to-primary-dark text-white text-sm font-semibold rounded-xl transition-all duration-200">
