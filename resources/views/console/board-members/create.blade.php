...
<div>
    ...
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 mb-6">
        <div class="flex items-center gap-3">
            ...
            <div>
                <h2 class="text-base font-extrabold text-gray-900">Tambah Pengurus Baru</h2>
                <p class="text-xs text-gray-400">Tambahkan data pengurus GPS TangSel</p>
            </div>
        </div>
        <a href="{{ route('console.board-members.index') }}" class="inline-flex items-center gap-1.5 px-4 py-2 text-sm font-medium text-gray-500 hover:text-gray-900 hover:bg-gray-100 rounded-xl transition-colors duration-200">
    ...
    <form action="{{ route('console.board-members.store') }}" method="POST" enctype="multipart/form-data" onsubmit="this.querySelectorAll('button[type=submit]').forEach(b => { b.disabled = true; b.classList.add('opacity-60', 'cursor-not-allowed'); b.querySelector('.btn-text').textContent = 'Menyimpan...'; })">
        @csrf
                ...
                <div class="bg-white rounded-2xl border border-gray-200 p-5 sm:p-6 space-y-6">
                    <div class="flex items-center gap-2.5 mb-2 border-b border-gray-100 pb-3">
                        <div class="w-8 h-8 rounded-lg bg-primary-light flex items-center justify-center flex-shrink-0">
                            <span class="material-symbols-rounded text-primary text-[18px]">person</span>
                        </div>
                        <span class="text-sm font-bold text-gray-800">Data Pengurus</span>
                    </div>

                    {{-- Nama --}}
                    <div>
                        <div class="flex items-center gap-2 mb-2">
                            <label for="name" class="text-xs font-bold text-gray-700">Nama <span class="text-red-500">*</span></label>
                        </div>
                    ...
                    <div>
                        <div class="flex items-center gap-2 mb-2">
                            <label for="position" class="text-xs font-bold text-gray-700">Jabatan <span class="text-red-500">*</span></label>
                        </div>
                        <input type="text" id="position" name="position" value="{{ old('position') }}" required
                            class="w-full px-4 py-3 rounded-xl border @error('position') border-red-300 bg-red-50 @else border-gray-200 @enderror text-sm text-gray-900 placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-primary/20 focus:border-primary transition-all"
                    ...
                    <div>
                        <div class="flex items-center gap-2 mb-2">
                            <label for="group" class="text-xs font-bold text-gray-700">Kelompok <span class="text-red-500">*</span></label>
                        </div>
                        <div class="relative">
                            <select id="group" name="group" required
                                class="w-full pl-4 pr-10 py-2.5 rounded-xl border @error('group') border-red-300 bg-red-50 @else border-gray-200 @enderror text-sm text-gray-900 focus:outline-none focus:ring-2 focus:ring-primary/20 focus:border-primary transition-all bg-white appearance-none">
                                <option value="">Pilih Kelompok</option>
                                <option value="pembina" {{ old('group') === 'pembina' ? 'selected' : '' }}>Dewan Pembina</option>
                                <option value="pengawas" {{ old('group') === 'pengawas' ? 'selected' : '' }}>Dewan Pengawas</option>
                                <option value="pengurus_harian" {{ old('group') === 'pengurus_harian' ? 'selected' : '' }}>Dewan Pengurus Harian</option>
                            </select>
                    ...
                    <div>
                        <div class="flex items-center gap-2 mb-2">
                            <label for="phone" class="text-xs font-bold text-gray-700">Nomor Telepon / WA</label>
                        </div>
                        <input type="text" id="phone" name="phone" value="{{ old('phone') }}"
                            class="w-full px-4 py-3 rounded-xl border @error('phone') border-red-300 bg-red-50 @else border-gray-200 @enderror text-sm text-gray-900 placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-primary/20 focus:border-primary transition-all"
            ...
            <div class="space-y-5">
                ...
                <div class="bg-white rounded-2xl border border-gray-200 p-5 sm:p-6">
                    <div class="flex items-center gap-2.5 mb-4 border-b border-gray-100 pb-3">
                        ...
                        <span class="text-sm font-bold text-gray-800">Foto Profil</span>
                    </div>
                    ...
                    <div class="space-y-4">
                        ...
                        <div class="border-2 border-dashed border-gray-200 rounded-xl p-4 flex items-center justify-center min-h-[140px] bg-gray-50/50 overflow-hidden" id="gambar-preview">
                            <div class="text-center" id="preview-placeholder">
                                ...
                                <p class="text-[11px] font-semibold text-gray-700">Belum ada foto</p>
                            </div>
                        </div>
                ...
                <div class="bg-white rounded-2xl border border-gray-200 p-5 sm:p-6">
                    ...
                    <div class="space-y-3">
                        ...
                        <a href="{{ route('console.board-members.index') }}"
