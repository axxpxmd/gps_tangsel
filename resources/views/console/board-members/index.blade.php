...
<div>
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-6">
        <div>
            <h2 class="text-lg font-extrabold text-gray-900">Daftar Pengurus</h2>
            <p class="text-xs text-gray-400 mt-0.5">{{ $members->count() }} pengurus tersimpan</p>
        </div>
        <a href="{{ route('console.board-members.create') }}" class="inline-flex items-center justify-center gap-2 px-5 py-2.5 bg-primary text-white text-sm font-semibold rounded-xl hover:bg-primary-dark transition-colors duration-200 shadow-sm shadow-primary/20">
                    ...
                    <tbody class="divide-y divide-gray-50">
                        @foreach ($members as $member)
                            <tr class="hover:bg-gray-50/50 transition-colors duration-150">
                                ...
                                <td class="px-5 py-4"><span class="text-xs font-medium text-gray-600">{{ $member->position }}</span></td>
                                <td class="px-5 py-4">
                                    @php
                                    ...
                                    @endphp
                                    <span class="inline-flex items-center px-2.5 py-1 rounded-full text-[10px] font-semibold {{ $groupColors[$member->group] ?? 'bg-gray-100 text-gray-500' }}">{{ $groupLabels[$member->group] ?? $member->group }}</span>
                                </td>
