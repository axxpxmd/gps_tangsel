...
@section('page_title', 'Edit Hadits')
...
<div>
    ...
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 mb-6">
        <div class="flex items-center gap-3">
            ...
            <div>
                <h2 class="text-base font-extrabold text-gray-900">Edit Hadits</h2>
                <p class="text-xs text-gray-400 line-clamp-1">{{ $hadits->source }}</p>
            </div>
        </div>
        <a href="{{ route('console.hadits.show', $hadits) }}" class="inline-flex items-center gap-1.5 px-4 py-2 text-sm font-medium text-gray-500 hover:text-gray-900 hover:bg-gray-100 rounded-xl transition-colors duration-200">
    ...
    <form action="{{ route('console.hadits.update', $hadits) }}" method="POST" onsubmit="this.querySelectorAll('button[type=submit]').forEach(b => { b.disabled = true; b.classList.add('opacity-60', 'cursor-not-allowed'); b.querySelector('.btn-text').textContent = 'Menyimpan...'; })">
        @csrf
        @method('PUT')
            ...
            <div class="space-y-5">
                ...
                <div class="bg-white rounded-2xl border border-gray-200 p-5 sm:p-6">
                    ...
                    <label class="relative inline-flex items-center cursor-pointer">
                        ...
                        <input type="checkbox" name="is_active" value="1" {{ old('is_active', $hadits->is_active) ? 'checked' : '' }} class="sr-only peer">
                ...
                <div class="bg-white rounded-2xl border border-gray-200 p-5 sm:p-6">
                    ...
                    <div class="space-y-3">
                        ...
                        <a href="{{ route('console.hadits.show', $hadits) }}"
