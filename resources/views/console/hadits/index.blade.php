...
<div>
    ...
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-6">
        <div>
            <h2 class="text-lg font-extrabold text-gray-900">Hadits Pilihan</h2>
            <p class="text-xs text-gray-400 mt-0.5">Kelola hadits pilihan GPS TangSel</p>
        </div>
        <a href="{{ route('console.hadits.create') }}" class="inline-flex items-center justify-center gap-2 px-4 py-2.5 bg-primary text-white text-sm font-semibold rounded-xl hover:bg-primary-dark transition-colors duration-200 w-full sm:w-auto">
                ...
                <table class="w-full min-w-[900px]">
                    <thead>
                        <tr class="border-b border-gray-100 bg-gray-50/80">
                            ...
                            <th class="text-left px-5 py-3.5 text-[11px] font-bold text-gray-400 uppercase tracking-wider">Sumber</th>
                    ...
                    <tbody class="divide-y divide-gray-50">
                        @foreach ($hadits as $item)
                            <tr class="hover:bg-gray-50/50 transition-colors duration-150">
                                ...
                                <td class="px-5 py-4">
                                    <a href="{{ route('console.hadits.show', $item) }}" class="text-sm font-bold text-gray-900 hover:text-primary transition-colors duration-200 font-arabic leading-loose line-clamp-2" dir="rtl">{{ \Illuminate\Support\Str::limit($item->arabic_text, 80) }}</a>
                                </td>
                                <td class="px-5 py-4">
                                    <p class="text-xs text-gray-500 line-clamp-2 max-w-[250px]">{{ \Illuminate\Support\Str::limit($item->translation, 100) }}</p>
                                </td>
                                <td class="px-5 py-4">
                                    <span class="inline-flex items-center px-2.5 py-1 rounded-lg bg-primary-light text-xs font-medium text-primary">{{ $item->source }}</span>
                                </td>
                                ...
                                <td class="px-5 py-4 text-right">
                                    <div class="flex items-center justify-end gap-1">
                                        <a href="{{ route('console.hadits.show', $item) }}" class="p-2 rounded-lg text-gray-400 hover:text-primary hover:bg-primary/10 transition-colors duration-200" title="Lihat">
                                            <svg class="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
                                                <path stroke-linecap="round" stroke-linejoin="round" d="M2.036 12.322a1.012 1.012 0 010-.639C3.423 7.51 7.36 4.5 12 4.5c4.638 0 8.573 3.007 9.963 7.178.07.207.07.431 0 .639C20.577 16.49 16.64 19.5 12 19.5c-4.638 0-8.573-3.007-9.963-7.178z"/>
                                                <path stroke-linecap="round" stroke-linejoin="round" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z"/>
                                            </svg>
                                        </a>
                                        <a href="{{ route('console.hadits.edit', $item) }}" class="p-2 rounded-lg text-gray-400 hover:text-primary hover:bg-primary/10 transition-colors duration-200" title="Edit">
                                            <svg class="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
            ...
            @foreach ($hadits as $item)
                <div class="bg-white rounded-2xl border border-gray-200 p-4">
                    <div class="flex items-start justify-between gap-2 mb-2">
                        <a href="{{ route('console.hadits.show', $item) }}" class="text-sm font-bold text-gray-900 hover:text-primary transition-colors duration-200 font-arabic leading-loose line-clamp-2" dir="rtl">{{ \Illuminate\Support\Str::limit($item->arabic_text, 60) }}</a>
                        @if ($item->is_active)
                    ...
                    <p class="text-xs text-gray-400 line-clamp-2 mb-2">{{ \Illuminate\Support\Str::limit($item->translation, 100) }}</p>
                    <div class="flex items-center justify-between">
                        <span class="inline-flex items-center px-2 py-0.5 rounded-md bg-primary-light text-[10px] font-medium text-primary">{{ $item->source }}</span>
                        <div class="flex items-center gap-0.5">
                            <a href="{{ route('console.hadits.show', $item) }}" class="p-1.5 rounded-lg text-gray-400 hover:text-primary hover:bg-primary/10 transition-colors duration-200" title="Lihat">
                                <svg class="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
                                    <path stroke-linecap="round" stroke-linejoin="round" d="M2.036 12.322a1.012 1.012 0 010-.639C3.423 7.51 7.36 4.5 12 4.5c4.638 0 8.573 3.007 9.963 7.178.07.207.07.431 0 .639C20.577 16.49 16.64 19.5 12 19.5c-4.638 0-8.573-3.007-9.963-7.178z"/>
                                    <path stroke-linecap="round" stroke-linejoin="round" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z"/>
                                </svg>
                            </a>
                            <a href="{{ route('console.hadits.edit', $item) }}" class="p-1.5 rounded-lg text-gray-400 hover:text-primary hover:bg-primary/10 transition-colors duration-200" title="Edit">
                                <svg class="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
                                    <path stroke-linecap="round" stroke-linejoin="round" d="M16.862 4.487l1.687-1.688a1.875 1.875 0 112.652 2.652L10.582 16.07a4.5 4.5 0 01-1.897 1.13L6 18l.8-2.685a4.5 4.5 0 011.13-1.897l8.932-8.931zm0 0L19.5 7.125M18 14v4.75A2.25 2.25 0 0115.75 21H5.25A2.25 2.25 0 013 18.75V8.25A2.25 2.25 0 015.25 6H10"/>
                                </svg>
                            </a>
