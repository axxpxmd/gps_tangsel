            ...
            <div class="lg:col-span-2 space-y-6">
                ...
                <div class="py-6 border-t border-gray-200 space-y-6">
                    <h4 class="text-sm font-bold text-gray-900 flex items-center gap-2">
                        ...
                        KOMENTAR PEMBACA ({{ $comments->count() }})
                    </h4>
                    ...
                    @if (session()->has('jemaah'))
                        ...
                        @endphp
                        <form wire:submit.prevent="submitComment" class="space-y-4 bg-slate-50 p-5 rounded-xl border border-gray-200">
                            @if (session()->has('comment_success'))
                                <div class="p-3 bg-emerald-50 text-emerald-700 text-xs font-semibold rounded-lg border border-emerald-100">
                            ...
                            <div class="flex items-center justify-between">
                                <div class="flex items-center gap-2.5">
                                    @if ($jemaah['avatar'])
                                        <img src="{{ $jemaah['avatar'] }}" alt="{{ $jemaah['name'] }}" class="w-8 h-8 rounded-full border border-gray-200">
                                    @else
                                        <div class="w-8 h-8 rounded-full bg-slate-200 flex items-center justify-center text-xs font-bold text-slate-700">
                                            {{ substr($jemaah['name'], 0, 1) }}
                                        </div>
                                    @endif
                                    <div>
                                        <div class="text-xs font-bold text-gray-800">{{ $jemaah['name'] }}</div>
                                        <div class="text-[10px] text-gray-400 font-medium">{{ $jemaah['email'] }}</div>
                                    </div>
                                </div>
                                <button type="button" 
                                        onclick="event.preventDefault(); document.getElementById('logout-jemaah-form').submit();"
                            ...
                            <div>
                                <textarea wire:model="commentContent" placeholder="Bagikan opini Anda dalam kolom komentar..." rows="3" 
                                    class="w-full px-4 py-3 rounded-xl border border-gray-200 text-sm text-gray-900 focus:outline-none focus:ring-2 focus:ring-primary/20 focus:border-primary transition-all resize-none"></textarea>
                                @error('commentContent')
                                    <span class="text-red-500 text-xs mt-1 block">{{ $message }}</span>
                                @enderror
                            </div>

                            <button type="submit" class="px-5 py-2.5 bg-[#2F5FA3] hover:bg-[#244B82] text-white text-xs font-bold rounded-xl transition-colors duration-200">
                                KIRIM KOMENTAR
                            </button>
                        </form>

                        <form id="logout-jemaah-form" action="{{ route('auth.logout-jemaah') }}" method="POST" class="hidden">
                            @csrf
                        </form>
                    @else
                        <div class="bg-slate-50 p-6 rounded-xl border border-gray-200 text-center space-y-3">
                            <p class="text-xs font-bold text-gray-500 uppercase tracking-wider">Tulis Opini Anda</p>
                            <p class="text-sm text-gray-600">Silakan masuk menggunakan akun Google Anda untuk dapat mengirimkan komentar.</p>
                            <div class="pt-2">
                                <a href="{{ route('auth.google', ['redirect' => url()->current()]) }}" 
                                   class="inline-flex items-center gap-2.5 px-6 py-3 bg-white hover:bg-slate-50 text-gray-700 text-sm font-bold rounded-xl border border-gray-300 shadow-sm transition-all duration-200">
                    ...
                    <div class="space-y-4">
                        @forelse ($comments as $comment)
                            <div class="p-4 bg-white rounded-xl border border-gray-200 flex gap-4">
                                @if ($comment->avatar)
                                    <img src="{{ $comment->avatar }}" alt="{{ $comment->name }}" class="w-9 h-9 rounded-full border border-gray-200 flex-shrink-0 object-cover">
                                @else
                                    <div class="w-9 h-9 rounded-full bg-slate-100 text-slate-700 font-bold flex items-center justify-center flex-shrink-0 text-xs">
                                        {{ substr($comment->name, 0, 1) }}
                                    </div>
                                @endif
                                <div class="min-w-0 flex-1">
                                    <div class="flex items-center justify-between">
                                        <span class="text-xs font-bold text-gray-900">{{ $comment->name }}</span>
                                        <span class="text-[10px] text-gray-400">{{ $comment->created_at->locale('id')->diffForHumans() }}</span>
                                    </div>
                        ...
                        @empty
                            <div class="p-8 text-center text-gray-400 bg-slate-50/50 rounded-xl border border-dashed border-gray-200">
                                <p class="text-xs font-medium">Belum ada komentar untuk artikel ini. Jadi yang pertama berkomentar!</p>
                            </div>
                        @endforelse
                    </div>
                </div>
